...
logging.basicConfig(
    filename=Path(Path(__file__).parent, "star360_emailer.log"), level=logging.DEBUG
)
    ...
    def __init__(self, csv_path: Path, debug=True, skip=None):
        ...
        with open(csv_path, "r") as csvf:
            ...
            for name, username, password in rd:
                ...
                if st.grade_level == 4 and not st.homeroom == [
                    "DAmario, Danielle",
                    "McNeill, Kaity",
                ]:
    ...
    def send_emails(self):
        ...
        with Email() as eml:
            for i, st in enumerate(self.students):
                ...
                logger.info(
                    f"Email sent to {st.name} including username: {st.username}"
                )
